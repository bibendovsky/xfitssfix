...
#ifndef XFITSSFIX_STRING_INCLUDED
#define XFITSSFIX_STRING_INCLUDED
...
struct CharTraitsT
{
	static TChar* copy(TChar* dest, const TChar* src, std::size_t count)
	{
	...
	static std::size_t length(const TChar* s)
	{
		assert(s != nullptr);
	...
	static int compare(const TChar* s1, const TChar* s2, std::size_t count)
	{
...
public:
	...
	StringT(const value_type* c_string)
	{
		ctor_c_string(c_string);
	}

	StringT(const value_type* c_string, size_type c_string_length)
	{
	...
	StringT(StringT&& rhs) noexcept
	{
		move(rhs);
	}

	StringT& operator=(const value_type* c_string)
	{
	...
	StringT& operator=(StringT&& rhs) noexcept
	{
		if (std::addressof(rhs) != this)
		{
			move(rhs);
	...
	StringT& append(const value_type* c_string, size_type count)
	{
	...
	StringT& append(const value_type* c_string)
	{
		assert(c_string != nullptr);
	...
	StringT& append(const StringT& rhs)
	{
		return append(rhs.data(), rhs.size());
	}

	StringT& assign(const value_type* c_string, size_type count)
	{
		resize(count);
	...
	StringT& assign(const value_type* c_string)
	{
		const auto length = traits_type::length(c_string);
		return assign(c_string, length);
	}

	StringT& assign(const StringT& rhs)
	{
...
private:
	...
	void ctor_c_string_internal_buffer(const value_type* c_string, size_type length) noexcept
	{
		is_external_buffer_ = false;
	...
	void ctor_c_string_external_buffer(const value_type* c_string, size_type length)
	{
		is_external_buffer_ = true;
		capacity_ = length;
		length_ = length;
	...
	void ctor_c_string(const value_type* c_string, size_type c_string_length)
	{
	...
	void ctor_c_string(const value_type* c_string)
	{
		const auto length = traits_type::length(c_string);
		ctor_c_string(c_string, length);
	}

	void move(StringT& rhs)
	{
...
template<typename TChar >
inline StringT<TChar> operator+(const TChar* lhs, const StringT<TChar>& rhs)
{
...
template<typename TChar>
inline StringT<TChar> operator+(const StringT<TChar>& lhs, TChar rhs)
{
	auto result = StringT<TChar>{};
...
template<typename TChar>
inline StringT<TChar> operator+(const StringT<TChar>& lhs, const TChar* rhs)
{
	using Traits = typename StringT<TChar>::traits_type;
...
template<typename TChar>
inline StringT<TChar> operator+(const StringT<TChar>& lhs, const StringT<TChar>& rhs)
{
	auto result = StringT<TChar>{};
...
template<typename TChar>
inline StringT<TChar>& operator+=(StringT<TChar>& lhs, TChar rhs)
{
	return lhs.append(rhs);
}

template<typename TChar>
inline StringT<TChar>& operator+=(StringT<TChar>& lhs, const TChar* rhs)
{
	return lhs.append(rhs);
}

template<typename TChar, typename TStringViewCharTraits>
...
template<typename TChar>
inline StringT<TChar>& operator+=(StringT<TChar>& lhs, const StringT<TChar>& rhs)
{
	return lhs.append(rhs);
}

template<typename TChar>
inline bool operator==(const StringT<TChar>& lhs, const TChar* rhs)
{
	return lhs.compare(rhs) == 0;
}

template<typename TChar>
inline bool operator==(const StringT<TChar>& lhs, const StringT<TChar>& rhs)
{
	return lhs.compare(rhs) == 0;
}

template<typename TChar>
inline bool operator!=(StringT<TChar>& lhs, const TChar* rhs)
{
	return !(lhs == rhs);
}
...
#endif // !XFITSSFIX_STRING_INCLUDED
